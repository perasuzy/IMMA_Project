...
def xcalculate_order_A2(orders):
    # โหลดข้อมูล JSON
    ...
    # Model
    model = pulp.LpProblem("MinTotalI5", pulp.LpMinimize)
    ...
    # Objective: minimize sum(i5 * count)
    model += pulp.lpSum(r.get("i5",0) * x_vars[r["id"]] for r in rows)
